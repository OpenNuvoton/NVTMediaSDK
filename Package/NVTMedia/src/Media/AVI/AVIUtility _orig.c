...
)
{
	...
	fsFileSeek(psAVIHandle->i32FileHandle, psAVIHandle->u32CurrentPos, SEEK_SET);
	fsWriteFile(psAVIHandle->i32FileHandle, (uint8_t *)&dwFourCC, 4, &i32Size);
	psAVIHandle->u32CurrentPos = fsFileSeek(psAVIHandle->i32FileHandle, 0, SEEK_CUR);
...
)
{
	...
	fsFileSeek(psAVIHandle->i32FileHandle, psAVIHandle->u32CurrentPos, SEEK_SET);
	fsWriteFile(psAVIHandle->i32FileHandle, (uint8_t *)&u32Dw, 4, &i32Size);
	psAVIHandle->u32CurrentPos = fsFileSeek(psAVIHandle->i32FileHandle, 0, SEEK_CUR);
...
)
{
	...
	if (fsFileSeek(i32FileHandle, psAVIFileContent->u32AVICurrentPos, SEEK_SET) < 0)
		return 0;

	fsReadFile(i32FileHandle, (uint8_t *)pu32Dw, 4, &i32Size);
...
)
{
	...
	if(u32DwSize == 0)
		return 0;
	fsWriteFile(psAVIHandle->i32FileHandle, pV, u32DwSize, &i32Size);
	psAVIHandle->u32CurrentPos = fsFileSeek(psAVIHandle->i32FileHandle, 0, SEEK_CUR);
	return i32Size;
}
...
)
{
	...
	fsReadFile(i32FileHandle, pV, u32Size, &i32RetSize);
...
{
	int32_t i32Size = 0;
	fsWriteFile(psAVIHandle->i32TempFileHandle, pV, u32DwSize, &i32Size);
	psAVIHandle->u32TempPos = fsFileSeek(psAVIHandle->i32TempFileHandle, 0, SEEK_CUR);	
	return i32Size;
}
...
{
	int32_t i32Size = 0;
	fsReadFile(psAVIHandle->i32TempFileHandle, pV, u32DwSize, &i32Size);
	psAVIHandle->u32TempPos = fsFileSeek(psAVIHandle->i32TempFileHandle, 0, SEEK_CUR);	
	return i32Size;
}
...
//	int32_t i;
	...
	if(psAVIHandle->u32MBResvSize){
		fsWriteFile(psAVIHandle->i32FileHandle,"aaaa",4, &i32FileWriteSize);
		fsSetFileSize(psAVIHandle->i32FileHandle, NULL, NULL, 1024*1024*psAVIHandle->u32MBResvSize); // Reserved SD Card space 
		fsFileSeek(psAVIHandle->i32FileHandle, 0, SEEK_SET);		
	}
	...
	if(psAVIHandle->bIdxUseFile == TRUE){
		...
		if(psAVIHandle->u32MBResvSize){
			fsWriteFile(psAVIHandle->i32TempFileHandle,"aaaa",4, &i32FileWriteSize);
			fsSetFileSize(psAVIHandle->i32TempFileHandle, NULL, NULL, 1024);
		}
		fsFileSeek(psAVIHandle->i32TempFileHandle, 0, SEEK_SET);
	...
	if(psAVIHandle->bIdxUseFile == TRUE){
		if(psAVIHandle->u32FixedNo >= 2048){
			if((retval = AVIUtility_WriteTempVOID(psAVIHandle, (uint8_t *)(&psAVIHandle->sAVICtx.psaAVIdxChunkInfo[0]), sizeof(S_AVIFMT_AVIINDEXENTRY) * psAVIHandle->u32FixedNo)) != (sizeof(S_AVIFMT_AVIINDEXENTRY) * psAVIHandle->u32FixedNo)){
				...
				fsFileSeek(psAVIHandle->i32TempFileHandle, psAVIHandle->u32TempPos, SEEK_SET);
				ERROR("Write chunk info fail");
				return 0;
			}
			psAVIHandle->u32FixedNo = 0;
		}
...
//	uint32_t dw00db, dw01wb;
	...
	if((psAVIHandle->bIdxUseFile == TRUE) && (szTempFilePath == NULL)){
		ERROR("AVI temp file path is NULL \n");
		return;
	}
...
//	dw01wb = mmioFOURCC('0', '1', 'w', 'b');
	...
	fsSetFileSize(psAVIHandle->i32FileHandle, NULL, NULL,dw);

	if(psAVIHandle->bIdxUseFile == TRUE){
		char *szTempFileName = malloc(strlen(szTempFilePath) + 14);
		char szUnicodeFileName[512];

		fsCloseFile(psAVIHandle->i32TempFileHandle);

		if(szTempFileName == NULL){
			ERROR("Alloc temp file path buffer failed \n");
			return;
		}
...
)
{
	...
	if (fsFileSeek(i32FileNo, (unsigned int)aviIndexEntry, SEEK_SET) < 0) {
		i32ErrNo = ERR_AVIUTIL_IDX1_HEADER;
		goto End_ParseIdx1;
	}

	fsReadFile(i32FileNo, (unsigned char *)asAviEntryBuff, sizeof(S_AVIFMT_AVIINDEXENTRY), &i32Size);
	...
	i32Size = fsReadFile(i32FileNo, (unsigned char *)&asAviEntryBuff[1], (sizeof(S_AVIFMT_AVIINDEXENTRY) * (u32AVIEntryNum - 1)), &i32Size);
...
)
{
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeMOVI, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_LIST_MOVI_CHUNK;
		goto End_ParseListMovi;
	}

	if (AVIUtility_ReadDWORD(i32FileNo, &u32FourCC2, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_LIST_MOVI_CHUNK;
		goto End_ParseListMovi;
	}
...
)
{
	...
	if (fsFileSeek(i32FileNo, psAVIFileContent->u32AVICurrentPos, SEEK_SET) < 0) {
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeSTRL, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_AUDIO_STREAM_HEADER;
		goto End_ParseAudioStreamHeaderChunk;
	}
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeSTRH, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_AUDIO_STREAM_HEADER;
		goto End_ParseAudioStreamHeaderChunk;
	}
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeSTRF, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_AUDIO_STREAM_HEADER;
		goto End_ParseAudioStreamHeaderChunk;
	}
	...
	if (psAudioInfoHeader->u16FormatTag == ULAW_FMT_TAG) {
		...
		psMediaAttr->sAudioAttr.u32SmplNumPerChunk = 1024; // ???, I don't know.
	}
...
LIST:
	...
	if (u32FourCC == 0x5453494c) { // 'L', 'I', 'S', 'T' for ffmpeg encoder avi
		if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeLIST, psAVIFileContent) < 4) {
			i32ErrNo = ERR_AVIUTIL_JUNK_HEADER;
			goto End_ParseJunk;
		}
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeJUNK, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_JUNK_HEADER;
		goto End_ParseJunk;
	}
...
)
{
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32Size, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_VIDEO_STREAM_HEADER;
		goto End_ParseVideoStream;
	}
...
)
{
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeRIFF, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_MAIN_AVI_HEADER;
		goto End_ParseMainAvi;
	}
	...
	if (AVIUtility_ReadDWORD(i32FileNo, &u32SizeHDRL, psAVIFileContent) < 4) {
		i32ErrNo = ERR_AVIUTIL_MAIN_AVI_HEADER;
		goto End_ParseMainAvi;
	}
