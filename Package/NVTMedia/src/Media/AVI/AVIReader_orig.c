...
)
{
	if (psFrameInfo->eNALType & eNM_UTIL_H264_NAL_SPS) {
		...
		if(psMediaAttr){
			psMediaAttr->u32H264Profile = pu8BitStream[psFrameInfo->u32SPSOffset +
...
)
{
	...
	*ppMediaRes = NULL;
	...
	if(fsFileSeek(psMediaAttr->i32FD, 0, SEEK_SET) < 0) {
		return eNM_ERRNO_IO;
	}
	...
	for (u32Index = 0; u32Index < sAVIFileContent.u32arrCounter; u32Index++) {
		...
		if(fsFileSeek(psMediaAttr->i32FD, u32IndexOffset, SEEK_SET) < 0) {
			eRet = eNM_ERRNO_IO;
			goto AVIRead_Open_fail;
		}


		fsReadFile(psMediaAttr->i32FD, (uint8_t *) & sIndexEntry, sizeof(S_AVIFMT_AVIINDEXENTRY), &i32Size);
...
//			}
	...
	if(psAVIReadRes->u32VideoChunkNum && psVideoCtx->eVideoType == eNM_CTX_VIDEO_H264) {
		...
		if(fsFileSeek(psMediaAttr->i32FD, u32ChunkOffset, SEEK_SET) < 0){
			eRet = eNM_ERRNO_IO;
			goto AVIRead_Open_fail;
		}

		fsReadFile(psMediaAttr->i32FD, (uint8_t *)FrameHeader, PARSE_FRAME_LEN, &i32Size);
	...
	if(pbSeekable) {
		if(psAVIReadRes->eVideoType == eNM_CTX_VIDEO_H264){
			if (psAVIReadRes->pu8H264FrameType[u32ChunkID] == 0)	{
				...
				if (fsFileSeek(i32FileNo, u32ChunkOffset, SEEK_SET) < 0)
					return eNM_ERRNO_IO;

				fsReadFile(i32FileNo, (uint8_t *)FrameHeader, PARSE_FRAME_LEN, &i32Size);
...
)
{
	...
	fsReadFile(i32FileNo, psCtx->pu8DataBuf, psIndexEntry->dwChunkLength, &i32Size);
