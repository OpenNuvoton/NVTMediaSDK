...
)
{
    ...
    psFileEnt->u32FileSize = sFileState.n64FileSize;
    free(pchFileFullPath);
...
)
{
    ...
    memset(szLongName, 0, MAX_FILE_NAME_LEN / 2);
    fsAsciiToUnicode(pchFolder, szLongName, TRUE);
    memset(&tFileInfo, 0, sizeof(FILE_FIND_T));
    i32ReadDirRet = fsFindFirst(szLongName, pchFolder, &tFileInfo);
    if (i32ReadDirRet < 0)
    ...
    do
    {
        ...
        if (strncmp(szLongName, pchFilePrefixStr, strlen(pchFilePrefixStr)) == 0)
        {
            SortAndLinkExistFile(pchFolder, szLongName, pchFilePrefixStr, pchFileExtStr, pchInstIdxStr, u32InstIdxPos,
                                 psFileList);
        }
        i32Ret = fsFindNext(&tFileInfo);
    }
    while (i32Ret == 0);
...
)
{
    ...
    fsAsciiToUnicode(pchFileName, szUnicodeFileName, TRUE);
    if (fsGetFileStatus(-1, szUnicodeFileName, NULL, &sFileState) < 0)
        return NULL;

    NMLOG_INFO("New recorded file name %s, size %d \n", pchFileName, (uint32_t) sFileState.n64FileSize);
    ...
    while (psTempEnt)
    {
        ...
        {
            psTempEnt->u32FileSize = sFileState.n64FileSize;
...
//  GetFNPostIdx(&u64FNPostIdx, u32FNPostOffset);
    ...
    i32FD = fsOpenFile(szUnicodeFileName, NULL, O_CREATE | O_RDWR);

    if (i32FD > 0)
    {
        AppendFileList(i32FD, pchFilePath, pchFilePrefixStr, pchInstIdxStr, u32InstIdxPos, psFileList);
    }
#endif
